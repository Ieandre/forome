# Forome — document de conception (v2, sur Nostr)

> Statut : **pivot protocolaire assumé.** Le protocole signé maison est remplacé
> par Nostr. Ce document remplace la conception v1, archivée dans
> [`docs/spec-v1-protocole-maison.md`](docs/spec-v1-protocole-maison.md) — c'est
> encore elle que le code du dépôt implémente (voir
> [`docs/migration-nostr.md`](docs/migration-nostr.md) pour l'état des lieux et
> le chemin).
>
> Même règle que la v1 : on consigne les décisions **et les raisons**, y compris
> les décisions négatives, y compris celles qui sont inversées ici.
>
> Nom de produit : **Forome** — décidé le 2026-08-13, et c'est un renversement de
> cette même ligne, qui portait `khey·forum` en nom de produit et gardait `forome`
> pour le seul dépôt « tant qu'un renommage n'est pas décidé ». Le renommage est
> décidé, et dans l'autre sens : c'est le nom du dépôt qui gagne. Raison : deux
> noms pour une seule chose est un coût permanent (chaque titre, chaque log, chaque
> mention en réunion demande de savoir lequel on emploie) pour un bénéfice nul, et
> `khey` reste de toute façon présent là où il veut dire quelque chose — le pseudo
> par défaut d'un membre, `khey_<8 hex>` (§3.1), et « ↻ new khey » (§3.6).

> **Convention de lecture.** Les numéros de kinds et de NIP cités sont ceux du
> dépôt `nostr-protocol/nips` au 2026-08-10. Plusieurs (NIP-7D, NIP-22, NIP-17)
> sont récents et bougent encore : **revérifier contre le dépôt amont au moment
> d'implémenter**, jamais contre ce document.

---

## 0. Ce que le pivot change

Le pivot n'est pas un changement de bibliothèque, c'est un changement de qui
détient la vérité. En v1, un serveur principal ordonnait, horodatait et
certifiait ; l'incensurabilité était une *sortie de secours* (données
rehébergeables). En v2, il n'y a plus de centre : l'incensurabilité est la
propriété par défaut, et tout ce qui exigeait une autorité devient un problème.

| | v1 (protocole maison) | v2 (Nostr) |
|---|---|---|
| Format | CBOR déterministe maison, octets conservés | event JSON NIP-01, id recalculé |
| Signature | Ed25519 + X25519 | secp256k1 / Schnorr (BIP-340) |
| Identité | clé de compte + clés d'appareil révocables | **une seule clé**, non révocable |
| Ordre | `seq` attribué par le serveur, ordre total | `created_at` déclaré par l'auteur |
| Horloge | `rts` du serveur, fiable | aucune horloge fiable |
| Retrait | pierre tombale signée, purge des octets | **impossible** au-delà de son propre relais |
| Pseudo | unique, jamais réattribué | **non unique**, usurpable |
| Extensibilité | champs inconnus préservés | par les `tags` uniquement |
| Interop | aucune | tout client Nostr lit et écrit |
| Canonicalisation | ton problème (piège n°1) | spécifiée par NIP-01 |

**Ce qu'on gagne :** l'interopérabilité (le profil, les follows et les MP
existent déjà et te suivent hors de ton app), la disparition du piège n°1 comme
risque projet, la réplication gratuite sur des relais tiers, et une promesse
produit nette — personne ne peut retirer un message du réseau, pas même toi.

**Ce qu'on perd :** l'ordre total, l'horodatage fiable, la révocation de clé, la
numérotation des posts, l'unicité des pseudos, et le retrait dur. Les six
...
**Un forum généraliste à haut débit, à la culture 18-25, sans inscription, en
temps réel, sur un protocole que personne ne contrôle.**

Le flux et la culture du 18-25, l'anonymat de 4chan, l'incensurabilité d'un
protocole ouvert, et une UX temps réel que personne n'a construite sur Nostr.
...
Le critère de jugement de la v1 est conservé : une idée doit être (a)
techniquement difficile, (b) immédiatement ressentie sans explication, (c)
coûteuse à copier. Une idée qui échoue au (b) est de la plomberie et doit être
présentée comme telle.

**Le créneau réel** est le (b) sur Nostr. Le protocole est public depuis des
années ; ce qui n'existe pas, c'est un client Nostr qui donne la sensation d'un
forum vivant. L'avantage n'est pas cryptographique, il est ergonomique — et
c'est tant mieux, parce que c'est le seul des trois qu'on maîtrise.
...
avec les règles d'échappement fixées par NIP-01. Adressage par contenu, comme
en v1 : un event est citable et infalsifiable.

### 2.2 Le piège n°1 change de nature, il ne disparaît pas

La v1 posait : *signer des octets, jamais un objet ; ne jamais re-sérialiser
pour vérifier.* **Nostr inverse cette règle** — la vérification re-sérialise
nécessairement, puisque seuls les six champs voyagent et que l'id est recalculé.

Ce n'est pas une régression : NIP-01 spécifie la sérialisation exactement
...
1. **Les champs inconnus ne sont pas préservés.** Un champ ajouté à la racine de
   ...
   discipline v1 (« un client ancien doit garder intact ce qu'il ne comprend
   pas ») reste obligatoire, mais elle s'applique aux tags.
2. **Pas de nonce implicite.** Deux events identiques du même auteur à la même
   seconde ont le même id — c'est le piège n°3 de la v1, toujours vivant. Sur
   Nostr il se résout gratuitement : le tag `nonce` de la preuve de travail
...
### 2.4 Ce qui n'existe plus : `seq`, reçu, horloge

La v1 avait deux objets signés : l'enregistrement d'auteur et le **reçu
serveur** portant `seq` (ordre total) et `rts` (heure fiable). Sur Nostr il n'y
a qu'un objet, et l'auteur signe sa propre heure.
...
- **La numérotation des posts perd son fondement.** En v1, `#1204` était la
  position réelle dans un registre signé — un permalien et une preuve. Voir
  §6.4 : elle devient un index local, et le permalien devient l'id.
- **Le registre des prédictions tombe.** Il reposait entièrement sur `rts` :
  sans horodatage de confiance, une prédiction « datée » ne prouve plus rien. Il
  faut un tiers horodateur (ancrage OpenTimestamps, ou une attestation signée
  par ton relais avec son horloge). Reclassé hors v1, et honnêtement dégradé —
  c'était la fonctionnalité la plus originale de la v1.
- **L'arbitrage des révocations de clé disparaît** — il utilisait `rts` pour
  décider si une signature précédait une révocation. Voir §3.2.

### 2.5 Édition et suppression

- **Édition** : kinds 11 et 1111 ne sont pas remplaçables, donc rien n'écrase
  rien — bonne nouvelle, c'est la propriété que la v1 construisait à la main.
  Une correction est un nouvel event référençant l'ancien ; l'affichage montre
...
- La v1 conservait hash et signature après purge des octets, pour prouver
  « X a existé ici, retiré par M ». Sur Nostr c'est sans objet : rien n'est
  purgé, l'event est simplement ailleurs.

---

## 3. Identité

**Le principal risque produit du projet, et le point le plus dégradé par le
pivot.** La v1 avait deux étages de clés, une révocation par appareil et une
échelle de récupération à cinq niveaux. Nostr a une clé.
...
Deux règles non négociables, héritées de la v1 §3.8 :
...
dérivé de la clé (repris tel quel de la v1, zéro upload).

### 3.2 Une seule clé — fin des clés d'appareil

La v1 séparait clé de compte (l'identité, rarement utilisée) et clés d'appareil
(une par navigateur, révocables individuellement, autorisées par délégation
signée). Téléphone perdu → révocation, identité intacte.
...
distance via une connexion autorisée, révocable indépendamment. C'est
exactement le modèle à deux étages de la v1, reconstruit une couche plus haut.
Reclassé en V4 pour les power users, avec **NIP-07** (extension navigateur) en
échappatoire desktop dès la V2.
...
### 3.4 Échelle de récupération, révisée

| # | Cas | Mécanisme | vs v1 |
|---|---|---|---|
| 1 | un appareil encore valide | QR vers le nouvel appareil | ✔ équivalent |
| 2 | clé sauvegardée (fichier / gestionnaire de mots de passe) | import de la `nsec` | ✔ équivalent |
| 3 | sauvegarde chiffrée par phrase de passe, stockée où on veut (ton relais peut l'offrir sans jamais voir le clair) | déchiffrement client | ✔ équivalent au coffre v1 |
| 4 | signeur NIP-46 | la clé n'a jamais quitté le bunker | ✔ meilleur |
| 5 | rien | **identité perdue définitivement** | = v1 |

Le coffre chiffré de la v1 (Argon2id + XChaCha20-Poly1305, dérivation côté
client) est directement réutilisable au niveau 3 — c'est la seule brique crypto
de `packages/protocol` qui survit au pivot sans modification conceptuelle.

### 3.5 Le pseudo n'est plus unique

**C'est la perte la plus contre-intuitive, et elle touche la thèse.** La v1
garantissait l'unicité du pseudo et son non-réattribution (v1 §3.7 : « sinon
quelqu'un attend la perte d'un pseudo connu pour usurper dix ans de
réputation »).
...
### 3.6 Anonymat jetable — inversion assumée d'une décision v1

La v1 écartait explicitement l'anonymat total : *« détruit l'attachement au
pseudo, qui est le cœur de la culture »*. La v2 introduit un bouton
**« ↻ new khey »** — identité jetable en un clic.

L'objection de la v1 reste vraie et n'a pas été réfutée : une communauté de
comptes jetables n'accumule pas de réputation, donc pas de folklore, donc pas de
culture. Décision retenue et sa borne :
...
- **« new khey » est une action secondaire délibérée**, pas un mode. Elle sert
  les topics d'aveu et le besoin ponctuel de se détacher de son historique —
  c'est le « mode couvert ponctuel » que la v1 gardait déjà en réserve
...
## 4. Ce que la signature débloque, revisité

| Fonctionnalité | v1 | v2 sur Nostr | Priorité |
|---|---|---|---|
| **Citation vérifiable** | v1 | **meilleur** — vérifiable par n'importe quel client Nostr, pas seulement le nôtre | V2 |
| Censure prouvable | gratuit (pierre tombale) | **gratuit et plus fort** — l'event n'est pas retiré, il est ailleurs | acquis |
| Édition transparente | gratuit (format) | convention de client (§2.5) | V2 |
| Continuité de pseudo | v2 | **acquis** — le nom est un kind 0 modifiable, l'identité est la clé | V4 |
| Badge auto-vérifié | v2 | NIP-05 + liens croisés dans kind 0 | V4 |
| **Registre des prédictions** | **v1** | **cassé** — plus d'horloge fiable (§2.4). Nécessite un horodateur tiers | hors v1 |
| Citation d'échange | v2 | idem v1, chaîne d'ids | V4 |
| Engagement scellé | v2 | inchangé (commit-reveal ne dépend pas de l'horloge) | V4 |

La règle de présentation de la v1 est conservée mot pour mot : **la crypto est
invisible par défaut.** Le risque de cette direction est de produire un forum
...
puis les nouveaux events en direct. Trois souscriptions, comme les trois canaux
de la v1 :
...
### 5.2 Le tick de liste : le garder, en le déplaçant

La v1 avait une bonne idée qu'il serait dommage de perdre : **un instantané
calculé une seule fois au centre, diffusé identique à tout le monde.** Coût =
un calcul + N envois, pas N × événements ; la charge ne dépend quasiment plus
...
2. **retenu — l'indexeur publie le tick comme event remplaçable**, signé par sa
   ...
   reçoivent tous les mêmes octets. L'insight de la v1 est préservé, et il
   devient *vérifiable* : le tick est signé, donc attribuable, et plusieurs
   indexeurs concurrents peuvent publier le leur

Corollaire de la v1 conservé : **le personnel se superpose côté client.**
...
Inchangé sur le fond : trier par dernier message laisse **un seul « up »
remonter un topic mort**, et trois personnes suffisent à squatter le haut de la
liste. Tri par participants distincts, rythme, accélération. La même mesure sert
à la détection de raid.

Correction de la v1 conservée : la vélocité n'est **pas** un pilier, c'est de la
plomberie plus une fonction de tri.

### 5.4 L'indexeur n'est pas optionnel

La discussion le présentait comme « un petit indexeur optionnel ». C'est faux
pour l'écran principal, et il vaut mieux l'écrire maintenant :
...
La v1 avait déjà nommé ce problème (« le piège de l'index ») et sa réponse reste
la bonne, elle est même renforcée : **rendre l'index remplaçable.** Les données
étant auto-vérifiables, plusieurs indexeurs concurrents peuvent tourner dessus,
...
Cette section est **inchangée par le pivot** — c'est de l'UX pure, et c'est là
que se joue le produit. Elle est reprise de la v1 avec ses trois pièges, parce
qu'elle est déjà implémentée et validée dans `apps/web`.
...
2. **Images** — le pire, et central ici : un sticker qui finit de charger fait
   ...
   Nostr, les dimensions viennent du tag `imeta` (NIP-92, champ `dim`). Nuance
   par rapport à la v1 : `dim` est *déclaré par l'auteur* et signé par lui, mais
   aucun serveur ne le vérifie contre les octets. Le hash `x` du même tag permet
...
Mais la numérotation des posts doit être redéfinie. En v1, `#1204` était la
position dans le registre signé : un permalien, et une preuve. Sur Nostr il n'y
a pas de compteur (§2.4). Décision :
...
Ce point compte plus qu'il n'y paraît : le post numéroté est un **code de forum**
(cf. phpBB, JVC), pas un détail. Il reste affiché, il change juste de statut —
d'une preuve à une commodité d'affichage. Aucun élément d'UI ne bouge.
...
Référence : phpBB / JVC nocturne. Tout ce qui évoque Discord est un bug.
...
Répartition inchangée : **RisiBank = découverte** (recherche, collections, tags
— ils le font mieux), **nous = durabilité**.
...
Raison critique, adaptée : le hash est **dans l'event signé**. Si RisiBank
...
- **Pas d'auto-embed d'URL externes** dans *notre* client — sinon on recrée
  l'hébergement arbitraire et la fuite d'IP des lecteurs. Nuance v2 : ce n'est
  plus qu'une politique de rendu locale, un autre client Nostr fera ce qu'il
  veut des mêmes events.
...
### 9.1 Décomposition, révisée

| | Objectif | v1 | v2 sur Nostr |
|---|---|---|---|
| a | Résister à son propre hébergeur | oui (sortie de secours) | **oui, par défaut** |
| b | Empêcher la censure silencieuse | oui (pierres tombales signées) | **oui, trivialement** — rien n'est retiré |
| c | Résister aux injonctions légales | non | **non, et c'est plus grave** — voir 9.2 |
| d | Rendre tout retrait techniquement impossible | possible mais autodestructeur | **c'est devenu le défaut** |

Le pivot transforme (d) d'une option écartée en propriété structurelle. Il faut
donc reprendre l'argument de la v1 sur pourquoi (d) est dangereux, parce qu'il
n'a pas cessé d'être vrai.

### 9.2 L'argument de la v1 tient toujours, et il vise maintenant le projet
...
noyés** — cette dernière phrase était dans la v1 comme contre-exemple ; elle
décrit maintenant le terrain de jeu.
...
**Le point juridique, sans enrobage.** Le plancher non négociable de la v1
(contenu pédocriminel, apologie du terrorisme, doxxing → retirable
immédiatement) **n'est plus atteignable au niveau du réseau.** Ce qui reste
atteignable, et qui est ce sur quoi tu seras jugé :
...
être retiré du réseau ; ce que *nous* servons, nous en répondons ».** La
formulation v1 (« impossible en silence » plutôt que « impossible ») était plus
solide qu'une promesse d'absolu, et une promesse fausse décrédibilise le projet
le jour où il faut retirer quelque chose. À faire relire par un juriste avant
l'ouverture publique (§15.4), pas avant le premier commit.
...
Bénéfices, identiques à la v1 : c'est le maximum de parole compatible avec un
lieu vivable, **plusieurs régimes de modération coexistent sur les mêmes
données**, et l'opérateur n'a plus à trancher les débats de modération — la
position la plus tenable pour un projet solo. Le pivot rend ça meilleur, parce
que les listes et les labels sont maintenant des events signés qui suivent
l'utilisateur.

### 9.5 Détection de raid — priorité haute

Le 18-25 a produit du harcèlement organisé, avec condamnations. Annoncer « pas
de censure » attirera **en priorité les gens exclus d'ailleurs, avant la
communauté cible** — et le peuplement initial détermine la culture pour
toujours. Le pivot aggrave ce risque : la promesse d'incensurabilité est
maintenant vraie et affichée, donc elle attire plus fort.
...
En topic signalé : les clés hors WoT sont ralenties (PoW plus difficile
demandée par la policy du relais, ou débit limité), jamais bloquées. Repris de
la v1 : des habitués qui s'emballent ne sont pas un raid — pondérer
l'accélération par la part de comptes récents.

### 9.6 Modération par pontage — étagère

Inchangé : l'algorithme (factorisation matricielle à la Community Notes) est
simple, le mur est ailleurs. Il faut des votants qui se recouvrent (avec 200
...
n'a que si on a déjà réussi. **Mauvais candidat v1.**
...
[`docs/moderation-staff.md`](docs/moderation-staff.md) : chaîne de confiance en
...
### 10.1 NIP-17, et pourquoi c'est meilleur que la v1
...
Ce que le gift wrap achète, et que la v1 n'avait pas : **les métadonnées sont
masquées aux relais.** Qui parle à qui, et quand, n'est pas lisible — seul le
destinataire apparaît, en tag `p` sur l'emballage. La v1 chiffrait le contenu
mais laissait l'expéditeur et le destinataire en clair côté serveur.
...
- **Envoyer aussi un emballage à soi-même**, sinon l'expéditeur ne relit pas ses
  propres MP. C'est exactement l'insight « deux boîtes par message » de la v1,
  et NIP-17 le prescrit.
...
- **Pas de forward secrecy** (fuite de la clé = historique lisible) — même
  compromis que la v1, assumé et à afficher. Implémentable en un week-end au
  lieu de six mois.
- MP sur une liste de relais dédiée, séparée des relais de contenu.

### 10.2 Ce que le gift wrap casse

La v1 appliquait **le blocage des MP côté serveur** : le client ne devait jamais
recevoir un MP d'un compte bloqué. **C'est impossible ici**, et c'est la
contrepartie directe du gain de confidentialité : l'emballage est signé par une
...
- ancienneté : le plus ancien event connu de cette clé — **déclarative, pas
  vérifiable**, contrairement à la v1 où le `rts` du serveur la prouvait
...
Le kind 0 ne porte **pas** de clé de chiffrement séparée : contrairement à la v1
(Ed25519 pour signer + X25519 pour les MP), la même clé secp256k1 sert aux deux
via ECDH. Une clé de moins à gérer, une surface de compromission de plus.
...
**Pas d'amis réciproques** — décision v1 conservée : l'amitié réciproque coûte
cher (demandes, acceptations, états intermédiaires, visibilité) et JVC
fonctionne très bien sans. Suivre (asymétrique) et bloquer (kind 10000,
asymétrique, dur) suffisent.

Effet de bord Nostr à assumer : **la liste de follows est publique.** Qui tu
suis est lisible par tout le monde, pour toujours. Ce n'était pas le cas en v1.
...
**Nuance SSR à trancher (§15.1).** La v1 jugeait le SSR *indispensable* — un
forum se découvre par Google. L'app actuelle est en `ssr: false`. En v2 le SSR
ne peut pas rendre depuis les relais (trop lent, non déterministe) : il doit
rendre **depuis l'indexeur**. C'est faisable et ça donne les permaliens
indexables, mais ça met l'indexeur sur le chemin critique du premier rendu —
exactement ce que §5.4 voulait garder remplaçable. Arbitrage à faire consciemment.

**Ce que le pivot supprime du risque projet :** l'argument décisif de la v1 pour
Node+TS des deux côtés était la canonicalisation — une seule bibliothèque
partagée client/serveur pour éviter deux implémentations divergentes. Nostr
rend ça sans objet : le format est spécifié en amont, testé en amont, et le
relais peut être en C++ sans risque. Le piège n°1 de la v1 cesse d'être notre
problème. C'est le meilleur argument technique en faveur du pivot.
...
Conservées pour ne pas refaire le débat. **Les trois premières lignes sont des
inversions de la v1** et portent leur justification.

| Écarté / inversé | Raison |
|---|---|
| ~~Protocole signé maison~~ → **Nostr** | l'interop et la disparition du piège n°1 valent plus que l'ordre total et l'horloge fiable. Prix payé : §2.4, §3.2, §3.5, §9.2 |
| ~~Anonymat total écarté~~ → **identité anonyme par défaut** | le coût d'identité à l'entrée est le premier frein produit. Borné par §3.6 : le défaut est *persistant*, le jetable est secondaire |
| ~~Clés d'appareil révocables~~ → **une clé, transportée par QR** | Nostr n'offre pas la délégation. NIP-46 restaure le modèle en V4 ; d'ici là, perte d'appareil = compromission sans recours |
...
| **Amis réciproques** | coût élevé, JVC fonctionne sans. Suivre + bloquer suffisent |
...
- **Vérification client** : vérifier la signature de tout ce qui arrive, ou à la
  ...
  nettement plus coûteux à vérifier qu'Ed25519 en JS (ordre du milliseconde,
  contre les 50–100 µs sur lesquels la v1 s'appuyait pour écarter l'objection de
  performance). **À mesurer avant de décider**, pas à supposer : à 40 events/s
...
- **Qui modère ?** La question de la v1 reste ouverte et change de nature : il
  n'y a plus de retrait à autoriser, mais il y a des curateurs d'étiquettes
...
Rien n'en a été conçu, et le pivot n'y change rien :
...
**Cadre légal**, et le pivot le rend plus urgent qu'en v1 (§9.2) : statut
d'hébergeur pour le relais **et** pour l'indexeur, procédure de signalement,
mentions légales, personne physique ou structure. Une vraie consultation, pas
une intuition. Avant l'ouverture publique.

### 15.5 Risque n°1 — les gens

Inchangé, et toujours très loin devant tous les risques techniques. Le produit
est conçu pour le haut débit : **à trente utilisateurs, rien de ce qui précède ne
fonctionne** — le tri par vélocité n'a rien à classer, le lissage ne lisse rien,
le web of trust est vide, la culture n'existe pas.

- un forum vide se peuple **par niche**, jamais par généralité. « Le 18-25 mais
  mieux » n'attire personne ; un lieu pour une communauté précise et mal servie
  attire quinze personnes qui reviennent
- la technologie ne sera jamais l'avantage réel — et Nostr encore moins que le
  protocole maison, puisqu'il est disponible pour tout le monde. Ce qui protège
  un forum, c'est sa culture et ses habitués. Mais une technologie qui **sert**
...
| | Contenu | Ce que ça valide |
|---|---|---|
| **V1** | **client lecture seule** sur relais publics : vue 30/70, gel de la liste, fil live, topic pré-ouvert | l'UX, sans écrire une ligne de protocole. Le seul vrai avantage concurrentiel se teste en premier |
| **V2** | publication + identité locale (§3.1) + création de topics + citation | l'onboarding inversé, et le fait qu'on puisse poster sans compte |
| **V3** | notre relais strfry + policy + PoW + QR multi-appareil + indexeur (tick, vélocité, recherche) | la tenue à la charge et l'anti-spam |
| **V4** | profils, follows, WoT, MP NIP-17, NIP-46, NIP-05 | l'identité complète |

L'ordre est délibéré : **la V1 ne dépend de rien qu'on possède.** Sur des relais
publics, avec des events kind 11/1111 existants, on peut valider la sensation du
produit avant d'écrire une policy de relais ou un indexeur. Si la V1 ne donne
pas envie d'y rester, aucune des autres ne le sauvera.
